...
- The video data for each round are provided as a single sequence (and not individual motions). In case you are intereseted in having synchronized video and AMASS (joint and body) files, you should trim F_PGx_Subject_x_L.avi files into single motion video files using  `single_videos.m` function.
